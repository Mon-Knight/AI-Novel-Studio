...
5. 用户额外要求不得与【本次编译约束】中的“必须满足”和“禁止违反”冲突；其余情况下仍应尽量满足用户额外要求，但不得完全抛弃大纲主线。

{{#compiled_constraints}}
【本次编译约束（优先执行）】
{{compiled_constraints}}

约束按“必须满足 / 应尽量满足 / 禁止违反”排列。必须满足和禁止违反不得被用户额外要求覆盖。
{{/compiled_constraints}}
...
{{#current_adopted_content}}
【当前章节已采用正文（仅作连续性与安全重生成参考）】
{{current_adopted_content}}

不得将该正文直接覆盖为本次输出；本次输出只能作为候选 Artifact，须经用户确认后才可采用。
{{/current_adopted_content}}
