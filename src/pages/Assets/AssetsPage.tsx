...
function AssetsPage() {
  ...
  const assetCards: AssetCard[] = [
    { title: '基础设定', icon: '📖', count: selectedNovel ? '1' : '0', desc: '作品信息、世界背景', path: selectedNovelId ? `/novels/${selectedNovelId}` : '' },
    { title: '角色库', icon: '👥', count: stats.chars || '0', desc: '已确认角色与状态', path: selectedNovelId ? `/novels/${selectedNovelId}` : '' },
      ...
      <div className="detail-card" style={{ marginBottom: 16 }}>
        ...
        {novels.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 16, color: 'var(--color-text-muted)', fontSize: 13 }}>
            暂无作品，请先在首页创建
            <div style={{ marginTop: 8 }}><button className="btn btn-primary btn-sm" onClick={() => navigate('/')}>← 返回首页</button></div>
          </div>
        ) : (
          <select className="input" value={selectedNovelId} onChange={(e) => setSelectedNovelId(e.target.value)} style={{ width: '100%' }}>
            {novels.map((n) => <option key={n.id} value={n.id}>{n.title}（{n.genre || '未分类'}）</option>)}
          </select>
        )}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          {assetCards.map((card) => (
            <div key={card.title} className="detail-card" style={{ cursor: card.path ? 'pointer' : 'default', textAlign: 'center', opacity: card.path ? 1 : 0.5 }}
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--color-primary)', marginBottom: 4 }}>{card.count}</div>
