...
const BUILTIN_TEMPLATES: BuiltInTemplate[] = [
  ...
  },
  {
    ...
    title: '快节奏输出方案',
    genre: '通用',
    description: '适合快节奏故事的输出控制',
    content:
      '{ "paceLevel": "fast", "dialogueRatio": 0.4, "descriptionRatio": 0.2, "endingHookRequired": true, "targetWordCount": 3500 }',
  },
  {
    id: 'tpl-008',
    type: '输出控制',
    title: '厚重输出方案',
    genre: '通用',
    description: '适合史诗厚重风格的输出控制',
    content:
      '{ "paceLevel": "slow", "dialogueRatio": 0.25, "descriptionRatio": 0.45, "endingHookRequired": false, "targetWordCount": 5500 }',
  },
];
...
const TEMPLATE_TYPES: { value: TemplateType; label: string }[] = Object.entries(
  TemplateTypeLabels,
).map(([value, label]) => ({ value: value as TemplateType, label }));
    ...
    >
      ...
      {showForm && (
        <div
          ...
          style={{ marginBottom: 16, borderColor: 'var(--color-primary)' }}
        >
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: 10, color: 'var(--color-text-muted)' }}>
                      {tpl.source === 'user_imported' ? '📤 导入' : '✏️ 自建'}
                    </span>
                  </div>
                  ...
                  >
                    {tpl.description || tpl.content.slice(0, 60)}
                    {tpl.content.length > 60 && !tpl.description && '...'}
                  </div>
                  {tpl.tags.length > 0 && (
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleUse(tpl.content, tpl.name);
                      }}
                    >
                      📋 使用
                    </button>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {BUILTIN_TEMPLATES.filter(
              (t) => filter === '全部' || filter === '系统内置' || t.type === filter,
            ).map((tpl) => (
              <div
                key={tpl.id}
                className="detail-card"
              ...
              >
                ...
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    handleUse(tpl.content, tpl.title);
                  }}
                  style={{ width: '100%' }}
                >
                  📋 使用模板
