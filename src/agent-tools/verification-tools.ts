...
export async function verifyOutlineCompliance(
  ...
  AgentToolResult<{ details: VerificationDetail[]; passedCount: number; failedCount: number }>
> {
  ...
  try {
    ...
    if (novelId && draft.length > 0) {
      try {
        const { protagonistRepository } =
          await import('../services/database/protagonistRepository');
        ...
        if (protagonists && Array.isArray(protagonists) && protagonists.length > 0) {
          const names = (protagonists as Record<string, unknown>[])
            ...
            .filter(Boolean);
          if (names.length > 0) {
            const missingNames = names.filter((name) => !draft.includes(name));
