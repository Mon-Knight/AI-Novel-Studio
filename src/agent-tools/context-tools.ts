...
import type { AgentToolResult, AgentToolContext, AgentReadableSummary } from './tool-types';
...
export async function buildAgentReadableContext(
  ...
  if (pData?.novel) {
    ...
    lines.push(`【作品】${novel.title ?? '未命名'}  (ID: ${novel.id ?? '?'})`);
  ...
  if (cData?.chapter) {
    const ch = cData.chapter as Record<string, unknown>;
    if (
      typeof ch.targetWordCount === 'number' &&
      (ch.targetWordCount as number) > 0 &&
      typeof ch.wordCount === 'number' &&
      (ch.wordCount as number) === 0
    ) {
  ...
  return successResult(
    {
      title: `第 ${
        cData?.chapter ? ((cData.chapter as Record<string, unknown>).title ?? '?') : '?'
      } 章 生成上下文`,
      summary: lines.join('\n'),
