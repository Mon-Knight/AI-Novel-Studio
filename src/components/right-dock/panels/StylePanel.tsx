...
function StylePanel({ novelId, chapter, onStyleChange, onOutputChange }: StylePanelProps) {
  ...
  const handleAnalyzeStyle = async () => {
    ...
    try {
      ...
      setAnalyzeError(e.message || '风格分析失败');
    } finally {
  ...
  const handleSaveAsStyle = async () => {
    ...
    try {
      ...
      setAnalyzeError(e.message || '保存风格方案失败');
    }
  };
    ...
    <div>
      ...
      <div className="panel-section">
        ...
        <textarea
          ...
          value={analyzeText}
          onChange={(e) => { setAnalyzeText(e.target.value); setUseChapterContent(false); }}
        ...
        {analyzeError && <div style={{ fontSize: 12, color: 'var(--color-error)', marginBottom: 8 }}>{analyzeError}</div>}

        {/* 分析结果 */}
        {analyzeResult && (
          <div style={{ border: '1px solid var(--color-primary-light)', borderRadius: 6, padding: 10, marginTop: 8 }}>
            <div className="panel-field-label" style={{ fontWeight: 600, marginBottom: 6 }}>📊 分析结果</div>
            {analyzeResult.narrativePerspective && (
              <div style={{ fontSize: 12, marginBottom: 3 }}>👁️ 视角：{analyzeResult.narrativePerspective}</div>
            )}
            {analyzeResult.tone && (
            ...
            {analyzeResult.sentenceStyle && (
              <div style={{ fontSize: 12, marginBottom: 3 }}>✍️ 句式：{analyzeResult.sentenceStyle}</div>
            )}
            <div style={{ fontSize: 12, marginBottom: 3 }}>
              💬 对话比：{Math.round((analyzeResult.dialogueRatio ?? 0) * 100)}% · 🖊️ 描写比：{Math.round((analyzeResult.descriptionRatio ?? 0) * 100)}%
            </div>
            {analyzeResult.styleSummary && (
              <div style={{ fontSize: 12, color: 'var(--color-text-secondary)', lineHeight: 1.6, marginTop: 6, padding: 6, background: 'var(--color-bg-primary)', borderRadius: 4 }}>
                {analyzeResult.styleSummary}
              </div>
            )}
            <button
