...
function EventsPanel({ novelId, chapter }: EventsPanelProps) {
  ...
  const handleSuggestEvents = async () => {
    if (!novelId || !chapter || suggestionAbortRef.current) return;
    const controller = new AbortController();
    suggestionAbortRef.current = controller;
    setLoading(true);
    setError('');
    setStatusMessage('正在生成本章事件建议…');
    try {
      ...
      setSuggestions(list);
      setStatusMessage(`已生成 ${list.length} 条事件建议`);
    } catch (e: unknown) {
      if (controller.signal.aborted || isAiRequestCancelled(e)) {
        setStatusMessage('已停止生成事件建议');
      } else {
  ...
  const handleAdoptSuggestion = async (s: EventSuggestion) => {
    ...
    const ev = await chapterEventService.create({
      ...
      involvedCharacterIds: s.involvedCharacterIds,
      impact: s.impact,
      risk: s.risk,
      status: 'selected',
      source: 'ai_suggested',
          ...
          <div key={ev.id} className="event-item">
            ...
            <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
              <button
                ...
                onClick={() => handleSetStatus(ev.id, 'required')}
              >
                必须发生
              </button>
              <button
                className="btn btn-text btn-sm"
                onClick={() => handleSetStatus(ev.id, 'forbidden')}
              >
                禁止发生
              </button>
              <button
                className="btn btn-text btn-sm"
                onClick={() => handleSetStatus(ev.id, 'selected')}
              >
                已选择
              </button>
              <button className="btn btn-text btn-sm" onClick={() => handleRemove(ev.id)}>
                删除
              </button>
            </div>
          </div>
          ...
          <div key={i} className="event-item" style={{ borderColor: 'var(--color-primary-light)' }}>
            ...
            <div
              style={{
                ...
                whiteSpace: s.rawText ? 'pre-wrap' : undefined,
              }}
            >
