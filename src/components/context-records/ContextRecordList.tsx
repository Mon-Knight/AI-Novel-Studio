...
function ContextRecordList({
  ...
  if (records.length === 0 && !compact) {
    return (
      <div
        style={{ padding: 12, fontSize: 13, color: 'var(--color-text-muted)', textAlign: 'center' }}
      >
        暂无上下文记录
        {onAdd && (
      ...
      {records.map((r) => (
        <div
          ...
          style={{
            ...
            cursor: 'pointer',
            borderLeft: `3px solid ${ContextRecordTypeColors[r.contextType] || 'var(--color-text-muted)'}`,
          }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                <span
                  style={{
                    ...
                    borderRadius: 3,
                    background:
                      (ContextRecordTypeColors[r.contextType] || 'var(--color-text-muted)') + '20',
                    color: ContextRecordTypeColors[r.contextType] || 'var(--color-text-muted)',
                    fontWeight: 500,
                  }}
                >
