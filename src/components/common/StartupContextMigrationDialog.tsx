...
function totalMigrated(migration: StartupContextMigrationState): number {
  return migration.chapterSummaries.inserted
...
function StartupContextMigrationDialog({ migration }: StartupContextMigrationDialogProps) {
  ...
  const shouldOpen = Boolean(migration.error)
  ...
  return (
    <div
      ...
      data-migration-status={failed ? 'failed' : migration.warnings.length > 0 ? 'warning' : 'migrated'}
    >
      <div className="modal-dialog" style={{ maxWidth: 520 }}>
        ...
        {migration.warnings.length > 0 && (
          <div
            ...
            style={{ marginTop: 12, color: 'var(--color-warning)', whiteSpace: 'pre-wrap', fontSize: 12 }}
          >
