...
function WorkspaceLeaveDialog({
  ...
  return (
    <ModalFrame
      title={saveDisabled ? '正文暂时无法完整读取' : '正文尚未保存'}
      titleId="workspace-leave-title"
      className="workspace-leave-dialog"
      overlayProps={{ className: 'workspace-leave-overlay' }}
      dialogProps={{ 'data-testid': 'workspace-leave-dialog', 'data-leave-reason': request.reason }}
      onDismiss={onCancel}
      busy={busy}
      dismissOnBackdrop={false}
      initialFocusSelector='[data-testid="workspace-leave-cancel"]'
      footer={actions}
    >
      {saveDisabled ? (
        <>
