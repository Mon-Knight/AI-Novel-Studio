...
function RecoveryDialog({
  ...
  const actions = (
    <>
      {!conflict && (
        <button className="btn btn-primary btn-sm" onClick={() => void onRestore()} disabled={busy}>
          恢复
        </button>
      )}
      ...
      <button className="btn btn-danger btn-sm" onClick={() => void onDiscard()} disabled={busy}>
        放弃恢复
      </button>
    </>
  );
  return (
    <ModalFrame
      ...
      overlayProps={{ className: 'workspace-recovery-overlay' }}
      dialogProps={{ 'data-testid': 'workspace-recovery-dialog' }}
      maxWidth={640}
      onDismiss={onLater}
      busy={busy}
