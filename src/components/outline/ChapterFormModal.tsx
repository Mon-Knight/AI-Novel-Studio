...
interface ChapterFormModalProps {
  ...
  volumeId?: string;
  volumes: Volume[];
  onSave: (input: CreateChapterInput | UpdateChapterInput) => void | Promise<void>;
  onClose: () => void;
}
...
function ChapterFormModal({
  ...
  const handleSave = async () => {
    ...
    if (title.trim().length > 80) {
      setError('章节标题不超过 80 字');
      return;
    }
    if (targetWordCount > 0 && (targetWordCount < 500 || targetWordCount > 20000)) {
      setError('目标字数建议 500-20000');
      return;
    }
    setSaving(true);
    try {
      ...
      setError(cause instanceof Error ? cause.message : '保存章节失败，请重试');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <ModalFrame
      title={initial ? '编辑章节' : '新建章节'}
      maxWidth={560}
      onDismiss={onClose}
      busy={saving}
      initialFocusSelector='input[aria-label="章节标题"]'
      footer={
        <>
