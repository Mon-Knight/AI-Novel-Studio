...
function getAll(): PolishRecord[] { return lsGet<PolishRecord[]>(KEY) ?? []; }
function saveAll(v: PolishRecord[]): void { lsSet(KEY, v); }
...
export const polishService = {
  ...
  async create(input: CreatePolishRecordInput): Promise<PolishRecord> {
    ...
    const r: PolishRecord = { ...input, id: generateId(), status: 'pending', createdAt: now, updatedAt: now };
  ...
  async update(id: string, patch: Partial<PolishRecord>): Promise<PolishRecord | null> {
    ...
    list[idx] = { ...list[idx], ...patch, updatedAt: nowISO() }; saveAll(list); return list[idx];
  },
