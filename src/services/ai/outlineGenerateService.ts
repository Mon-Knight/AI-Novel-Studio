...
async function buildOutlineContext(novelId: string, volumeId?: string) {
  ...
  try {
    ...
    if (activeStyle) {
      const parts: string[] = [];
      if (activeStyle.narrativePerspective)
        parts.push(`叙事人称：${activeStyle.narrativePerspective}`);
      if (activeStyle.tone) parts.push(`文风：${activeStyle.tone}`);
      if (activeStyle.pace) parts.push(`节奏：${activeStyle.pace}`);
      parts.push(
        `对话比例：${Math.round(activeStyle.dialogueRatio * 100)}%，描写比例：${Math.round(activeStyle.descriptionRatio * 100)}%`,
      );
      ...
      if (activeStyle.prohibitedStyles?.length)
        parts.push(`禁用：${activeStyle.prohibitedStyles.join('、')}`);
  ...
  return {
    ...
    protagonist: protagonist
      ? [protagonist.name, protagonist.identity, protagonist.personality, protagonist.goal]
          .filter(Boolean)
          .join('；')
      : undefined,
...
export const outlineGenerateService = {
  async generateNovelOutline(novelId: string, options: AiGenerateOptions = {}): Promise<string> {
  ...
  async generateVolumeOutline(
    input: { novelId: string; volumeTitle?: string },
    options: AiGenerateOptions = {},
  ): Promise<VolumeOutlineCandidate> {
    const settings = aiSettingsService.getSettings();
    ...
    const task = await aiTaskService
      .create('volume_outline_generate', {
  ...
  async generateChapterOutlines(
    ...
    const [context, volume] = await Promise.all([
      ...
      input.volumeId
        ? volumeRepository.getById(input.volumeId).catch(() => null)
    ...
    try {
      ...
      const response = await client.generate(request, options);
      throwIfAiRequestCancelled(options.signal);
      const parsed = safeJsonParse<{ chapters: ChapterOutlineCandidate[] }>(response.text, {
        chapters: [],
      });
      const chapters = Array.isArray(parsed.chapters)
        ? parsed.chapters
            .filter((item) => item.title && item.outline)
            .map((item) => ({
              ...item,
              targetWordCount:
                Number.isFinite(item.targetWordCount) && (item.targetWordCount as number) > 0
      ...
      if (context.activeMasterOutlineId)
        usedOutlines.push(`总纲:${context.activeMasterOutlineId.slice(0, 8)}`);
      if (context.activeVolumeOutlineId)
        usedOutlines.push(`分卷大纲:${context.activeVolumeOutlineId.slice(0, 8)}`);
