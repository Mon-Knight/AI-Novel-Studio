...
function descriptorFor(manifest: ToolRegistryManifestV1, identity: string): ToolDescriptorV1 {
...
export async function verifyChapterReadinessPlan(
  ...
  if (
    bundle.plan.contractVersion !== 'agent_plan_v1' ||
  ...
  for (let index = 0; index < expectedSteps.length; index += 1) {
    ...
    if (
      actualDependencies.some((ordinal) => ordinal === undefined) ||
      actualDependencies.length !== expectedOrdinals.length ||
      actualDependencies.some(
        (ordinal, dependencyIndex) => ordinal !== expectedOrdinals[dependencyIndex],
      )
    ) {
