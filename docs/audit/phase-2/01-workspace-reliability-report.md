...
本报告最初对应从旧开发基线形成的 `feat/v2.2.0-workspace-reliability` 实现。正式合入时没有覆盖后续主线，而是在已发布的 `v2.1.8`（`854e49d`）上协调移植 v2.2.0 工作区可靠性提交，并保留 v2.1.2～v2.1.8 已有的数据安全、任务恢复、请求取消、质量历史与章节上下文持久化能力。

协调后的关键兼容结果包括：

- 启动入口同时保留旧上下文迁移、生成任务启动恢复和 v2.2.0 路由级 Leave Guard；
- 正文编辑器同时保留文档迟到读取隔离、`DraftContentState`、保存防重入、恢复快照和正文不可用锁定；
- 草稿正式采用继续执行章节上下文过期与浏览器补偿回滚，同时统一到 v2.2.0 原子草稿保存入口；
