...
2. **跨进程全书调度与三档策略已经实现**：migration 027 以 SQLite 保存 run、lease、attempt 和 append-only checkpoint，支持 `draft_night / quality_gate / full_auto`、重启恢复、预算、时间窗、重试与熔断；应用完全退出后的系统后台常驻仍是后续增强。
...
6. **成本计量与当前进程请求治理已实现**：用户可配置输入/输出 USD 单价，任务创建时冻结价格并按 Provider usage 结算；WebView 在派发前执行速率、并发、每日 Token / 成本硬预算和保守 reservation。Provider 账单对账与跨进程统一额度仍缺失。
...
| 10  | AI usage / 成本计量 | **估算与本地硬门禁已实现**   | **P0 收口**    | 预算、限流和 reservation 已实现；跨进程额度与账单对账仍缺失 |

当前工作树已完成取消、真实流式事件、成本估算与本地硬门禁、参考资料 / 分层风格、混合语义 Memory、跨进程 Autonomous Scheduler、通用多目标事务、章节 metadata 批处理和正式势力 / 地点资产的主要代码链路；本地 MSI/NSIS、真实浏览器 E2E 与 14 套 Windows 桌面 E2E 已通过。下一步增强 Provider 兼容、系统后台常驻、正文级批处理、全书分析和资产可视化；签名 updater 继续由 release workflow 注入密钥生成。
...
- EPUB 章节顺序稳定；PDF 提取质量不足时向用户显示明确警告。
...
应用启动时会把中断的 `running` run 收敛为 `queued`、active lease 收敛为 `expired`、claimed attempt 收敛为 `abandoned`，随后只接管后端确认可恢复的 run；旧 epoch 不会复活，也不会静默重放状态不明的 Provider 请求。
...
- 当前 worker 随桌面应用进程运行；应用完全退出后不会作为 Windows 服务或托盘守护进程继续调用 Provider。系统定时唤醒、托盘运行和凭据解锁生命周期仍需单独设计。
- migration 027 解决了单个 run 的跨进程 owner、恢复和冻结预算，但前台 AI 请求 ledger 与 scheduler ledger 尚未合并为跨全产品、跨进程的统一额度账本。
...
仍属后续增强：自动 embedding 供应商适配、冻结召回评估集、HNSW/SQLite vector 扩展和全书分析 UI；这些增强不得改变当前“无向量则明确降级、SQLite 为权威、旧采用稿失效”的契约。
...
- 当前已有小说作用域内的 FTS / substring、显式向量余弦与结构化过滤后端，但没有全书检索中心、角色出场统计、冲突 / 伏笔时间线、情绪聚合或节奏热力图页面。
...
- 每分钟频率与并发配额分别产生稳定错误；过期 reservation 会清理，成功响应按真实 usage 结算。
- 缺失 usage 的已完成响应保留缺失计数并采用保守额度，不把未知成本显示为零。
...
- cancelled/failed 请求可能已经产生 Provider 费用，但缺少最终 usage 时保持未知；没有服务端账单导入或差异对账。
- 前台治理 ledger 属于当前 WebView 的本地事实，Scheduler budget 属于单个持久 run；两者尚未合并为跨窗口、跨进程、跨 run 的全产品统一额度账本。
- 当前限流是用户配置的固定每分钟/并发门禁；Provider `Retry-After`、动态 429 退避、组织级配额同步、月度/项目聚合、超额审批和成本趋势告警仍待增强。
- 当前列表合计只累加已经 `complete` 的可见任务，不能解释为项目总支出。

### 后续增强门禁

- 跨进程统一预算必须继续复用“预估预留 → 实际 usage 结算 → 差额释放/补记”，并保持 operation/task/attempt 可追溯，不能削弱现有前台与 scheduler 硬门禁。
- `unpriced`、`usage_missing`、失败和取消必须单独统计，任何聚合报表都应显示未知覆盖率。
- 后台无人值守策略继续在派发前读取冻结预算；未来的组织级并发/速率治理必须由共享权威 ledger 仲裁，UI 自报额度不能授权 worker 越过硬门禁。
...
- 前台 request ledger 与 scheduler run ledger 都已执行硬门禁，但事实域仍分离；建设跨全产品额度时应以共享 reservation/CAS 账本协调，而不是简单相加 UI 统计。
...
| v3.1.x          | 自动 embedding 适配、Memory 召回评估与重建工具                                                                  | 100+ 章可追溯语义回忆的运维闭环      | L    |
| v3.1.x 后段     | 角色/冲突/伏笔/节奏确定性分析与热力图                                                                           | 全书分析工作台                       | L    |
| v3.2.x          | Windows 后台常驻、统一跨进程额度、凭据生命周期、通知与长时间耐久验证                                            | 从可恢复调度进入系统级夜间运行       | XL   |
...
| usage / 成本计量 | 4        | 2          | 5        | 3        | 本地硬预算已实现，继续做跨进程额度 / 对账  |
...
1. **真实正文流、当前进程请求控制、成本估算与本地硬预算已统一到基础协议，本地安装包、真实浏览器与完整桌面 E2E 已收口；后续继续扩展 Provider 兼容降级和逐入口 UI 证据；**
2. **参考资料、分层风格画像和 SQLite 混合语义 Memory 已形成代码、动态测试、安装包与桌面 E2E 闭环，并可继续增强自动 embedding、评估集与全书分析 UI；**
3. **migration 027 已建立跨进程调度、持久 owner/lease、run 预算、恢复和三档自动决策；剩余缺口是应用完全退出后的 Windows 后台常驻、统一额度账本、通知与耐久性验证；**
4. **migration 028 已建立多目标事务、跨章节 metadata 批处理和势力/地点正式实体模型；剩余缺口是正文级批处理、提交后 undo、旧条目迁移及关系图/地点地图体验。**

因此，下一阶段不再重复建设已经落地的 Scheduler、事务、正式资产 schema 或本地桌面门禁，而应基于现有协议补齐 Provider 兼容、系统后台常驻、正文级批处理、全书分析和势力/地点可视化；所有增强继续保持草稿安全、预算门禁与用户控制。
