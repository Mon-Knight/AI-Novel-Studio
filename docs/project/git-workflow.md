...
稳定/预发布更新索引按通道串行发布，并在实际更新前重新读取当前通道版本；旧版本不得覆盖较新指针，相同版本只允许相同内容的幂等重试。上一回滚版本必须属于相同通道且严格小于本次版本，不能按 Release 列表时间猜测。远程读取、鉴权或元数据解析错误须失败关闭，不误判为首次发布。多资产上传不是原子事务；失败时补偿旧元数据并读回复验，补偿失败必须显式报告。Fake transport 故障测试不等于真实签名、上传或生产通道发布成功。
