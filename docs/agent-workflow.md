...
已授权且明确的同会话任务不必再次生成任务书或等待确认；仅对尚未解决的关键架构取舍、破坏性操作授权、需求歧义和范围扩展询问。未运行或未通过的门禁不能写成通过；报告限制不等于豁免发布门禁。
...
| 前端/TypeScript  | 相邻/模块行为测试、改动文件零警告 lint、一次类型检查；交互变化加对应真实桌面场景，构建配置变化再加构建 |
| Rust/SQLite      | locked check、相关动态测试；版本验收完整串行 Rust 测试；不使用真实用户库                               |
| 桌面/DSH payload | 固定载体、当前 Gateway 与相关真实桌面场景；打包配置/产物变化或发布时运行 `npm run tauri:build`         |
| 发布             | `scripts/agent-workflow/verify_project.ps1` 完整矩阵；不能用定向测试替代                               |

日常入口是 `npm run verify:change`；`--dry-run` 显示选择理由，`--base <ref>` 纳入基线以来的提交与工作区修改。检查选择在本地与 PR CI 共用；已有相同代码/配置上的有效结果不重复运行，变更、失败修复或具体新风险只复测受影响项。未映射的代码需补充行为归属，不能空跑通过或自动改为全仓测试。功能开发脚本默认只做开工检查，`-Phase Verify` 才调用此入口。

最后执行 `git diff --check` 和 `git status --short`，确认没有混入构建产物、凭据、用户数据或无关文件。纯文档任务不要求完整 Tauri 发布构建，普通任务也不要求自动提交来取得 clean working tree。
