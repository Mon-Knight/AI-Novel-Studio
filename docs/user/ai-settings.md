...
1. 进入「设置中心」→「AI 模型配置」
2. 关闭 Mock 模式
3. 点击「添加来源」，填写 API 密钥；在「自定义设置」中填写显示名称、API 地址与协议（`deepseek` 或 `openai_compatible`）
4. 在「模型目录」中手动添加模型，或点击「获取可用模型」拉取该来源上游列表；需要时展开一行填写上下文窗口与最大输出 Token
5. 点击「保存」；已保存项按来源（Base URL + 密钥身份）显示为一张卡片，不展示密钥。点击卡片只选中该来源（高亮），只有「编辑」打开设置表单，再点一次「编辑」收起。「自定义设置」与模型参数默认收起，点开后再填写。「恢复默认模型」只还原本次打开编辑器时的目录
