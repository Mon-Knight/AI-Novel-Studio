...
| ------ | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| P0     | 全产品可靠取消、请求所有权、usage 与成本计量 | **已实现**：所有生产生成入口传播 signal/owner，迟到响应隔离；冻结单价、usage、速率/并发、每日 Token/成本硬预算和预警均已接入                | 成本是冻结单价的 USD 估算，不等同 Provider 最终账单                                 |
...
| P1     | 参考资料库、分层风格画像、混合语义 Memory    | **已实现**：TXT 版本库、六层采样、画像来源、adopted draft 绑定、同事务失效、FTS + 显式向量混合检索和审计均已完成                            | PDF/EPUB、自动 embedding 与召回评估集后续增强                                       |
...
- v3.1.x：自动 embedding、召回评估集、PDF/EPUB 参考资料和更精细的上下文压缩。
- v3.2.x：调度可观测性、凭据生命周期增强、更多故障恢复运营工具。
- v3.3.x：更多正式资产类型、正文候选批处理编排和全书分析 UI。
