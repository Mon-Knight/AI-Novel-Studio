...
| v2.1.2 | **当前** | **完整备份与恢复闭环** | 版本化完整项目备份、SQLite 单事务恢复、ID 重写、外键和大文本完整性校验，以及本地缓存恢复失败后的前端补偿 |
...
进入 v2.x 版本号不代表 Planner、Tool Calling、Memory 与 Verification 已全部实现；这些仍按独立版本目标逐步交付。v2.1.2 当前只处理完整备份与恢复闭环。
