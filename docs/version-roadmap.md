...
| v2.1.5 | **当前** | **章节工程任务跨重启恢复闭环** | 遗留非终态任务原子结算、稳定错误码、checkpoint 保留、状态机收紧和真实进程重启 E2E |
...
- 随后收敛真实网络取消、质量历史不可变快照、稳定重放与更完整的组件 / 桌面 E2E。
...
进入 v2.x 版本号不代表 Planner、Tool Calling、Memory 与 Verification 已全部实现；这些仍按独立版本目标逐步交付。v2.1.5 当前只发布章节工程任务的安全中断结算，不扩展 Agent 自主写入能力，也不宣称可以自动续跑。
