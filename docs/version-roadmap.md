...
| v2.1.1 | **当前** | **正文变更安全门** | 固定目标与基础版本、隔离迟到响应、冲突拒绝、原子采用、会话级幂等应用 |
...
进入 v2.x 版本号不代表 Planner、Tool Calling、Memory 与 Verification 已全部实现；这些仍按独立版本目标逐步交付。v2.1.1 当前只处理正文变更安全门。
