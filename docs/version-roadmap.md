...
| v2.1.8 | **当前** | **章节上下文持久化一致性闭环** | SQLite 桌面单一事实源、稳定上下文 ID、总结 / 上下文 / 角色状态 / 章节终态原子提交、旧缓存幂等迁移与重启 E2E |
...
进入 v2.x 版本号不代表 Planner、Tool Calling、Memory 与 Verification 已全部实现；这些仍按独立版本目标逐步交付。v2.1.8 当前只发布章节上下文持久化一致性闭环，不扩展 Agent 自主写入能力，也不宣称任务可以自动续跑、跨存储具备 ACID 或所有 AI 工具均可取消。
