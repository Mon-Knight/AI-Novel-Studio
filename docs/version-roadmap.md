...
v2.1.1  正文变更安全门
v2.1.2  完整备份与恢复闭环
v2.1.3  Windows 真实桌面 E2E 与稳定性（当前）
```
...
| v2.1.3 | **当前** | **Windows 真实桌面 E2E 与稳定性** | WebdriverIO 驱动真实 Tauri 窗口，隔离 SQLite / WebView2 / 进程，阻断真实 AI 与外部网络，并修复测试暴露的稳定性缺陷 |
...
进入 v2.x 版本号不代表 Planner、Tool Calling、Memory 与 Verification 已全部实现；这些仍按独立版本目标逐步交付。v2.1.3 当前只发布真实桌面验证与稳定性基础，不扩展 Agent 自主写入能力。
