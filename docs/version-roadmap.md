...
| v2.1.7 | **当前** | **章节质量历史不可变快照与原子重放** | report/items/state 单事务、历史只读回放、迟到竞态保护、AI Task 强绑定、schema 3 备份和真实桌面重启 E2E |
...
进入 v2.x 版本号不代表 Planner、Tool Calling、Memory 与 Verification 已全部实现；这些仍按独立版本目标逐步交付。v2.1.7 当前只发布章节质量历史的不可变快照、状态分离与重放闭环，不扩展 Agent 自主写入能力，也不宣称任务可以自动续跑或所有 AI 工具均可取消。
