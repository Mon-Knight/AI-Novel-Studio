...
pub fn list_style_profiles(
    ...
    let sql = match project_id {
        Some(_) => format!(
            "{STYLE_PROFILE_SELECT} WHERE novel_id = ?1 ORDER BY is_active DESC, updated_at DESC"
        ),
...
pub fn find_active_style_profile(
    ...
    let sql = format!(
        "{STYLE_PROFILE_SELECT} WHERE novel_id = ?1 ORDER BY is_active DESC, updated_at DESC LIMIT 1"
    );
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
...
pub fn deactivate_all_style_profiles_for_novel(
    conn: &Connection,
    novel_id: &str,
) -> Result<(), String> {
    conn.execute(
