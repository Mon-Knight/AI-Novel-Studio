...
- **Writing SubAgent 正式开放（桌面端 + 真实 API 模型默认启用）**：`chapter_write` 意图在桌面端且冻结模型为 `runtimeMode=api` 时默认经 Writing SubAgent 走 DSH——模型在只读工具 + 唯一候选工具 `generate_chapter` 的收窄 allowlist 内自主读取上下文并提交 candidate-only 章节正文，宿主校验作用域、必需读取顺序、字数硬区间与产物契约，候选仍只进入人工审阅、`ReviewAuthorization` 与原子采用链路；mock / 本地模型、浏览器模式与未绑定章节的目标继续走确定性 Writer。`localStorage['ai_novel_studio_writing_subagent_dsh']='0'` 可显式关闭，`'1'` 仍可强制开启（供 opt-in 验收载体使用）。放行依据：E-4a 客户端 Gemini 卡片正向真实验收与 E-4b 六个负例/恢复场景全部通过，验收发现的 GAP-18 / GAP-19 已在本版修复；真实配置验收改为不设置开关、按默认路径复验。

- **桌面 Agent 工作台式 UI 重构（ZCode 形态）**：设计令牌改为深色优先的低饱和中性面板、细边框、6～16px 圆角与克制阴影（保留全部既有令牌名，浅色/深色共用语义令牌，新增 `--color-bg-selected / -elevated / -composer / -frame`、`--frame-bar-height`、`--side-panel-width`）。壳层新增常驻顶部标签栏 `FrameBar`（会话 / 项目 / 当前页面标签、收起侧栏、版本与资源中心入口）；全局侧栏改为快捷操作（新建任务 `Ctrl+N`、搜索 `Ctrl+K`、自主创作、项目库）+ 分组导航（工作区 → 会话 / 项目 / 资源中心；进入作品后追加“当前项目”七页）+ 底部身份行，快捷操作通过路由 state 把意图交给工作台。创作工作台：项目/任务树承担同一套快捷操作与身份行，任务行只保留标题、相对时间与状态色点；对话流居中、用户回合卡片气泡、运行块与产物卡改 12px 圆角卡；输入框改为悬浮圆角卡片（上下文 pill 左置，模型 pill 与圆形发送键右置）；新增右侧“打开标签页”面板（当前插件 / 章节审阅 / 作品概览 / AI 任务记录），插件面板改在同一列内打开；任务创建提交逻辑抽到 `workbenchTaskSubmission.ts`。新增 `ProjectPageFrame`（作品下六个页面共用横向标签条）与资源中心 `HubLayout / HubSidebar`（风格方案、模板中心、创作资产、导入导出、AI 任务记录与五类设置共用分组左导航，设置分类由 `?tab=` 驱动）。所有路由、`data-testid`、IPC 与授权/事务边界保持不变；移除未再使用的 `TopBar`。同步 README 页面入口与 UI 参考 §18.11，浏览器布局规格按新壳层重新标定（树宽 248px、资源中心导航 232px、标签栏 38px）。
- **工作台体验收口（P1 第一批）**：模型目录不可用提示改为紧凑状态条（一行摘要 + 展开说明 + 同排「新建任务 / 重试 / 设置」动作，可访问名称与 `data-testid` 不变），模型 pill 增加可用/刷新/不可用状态点；输入卡新增「+」插入菜单（`workbench-composer-attach`），任务模板与「查看当前插件」收纳其中，菜单关闭时模板控件仍在 DOM（`hidden`），上下文 pill 保留在工具栏；右侧面板由启动卡扩展为可内联打开的真实工具——创作上下文（复用 `WorkbenchAssetScopePanel`，独立 id）、本任务产物（按时间倒序，点击定位并高亮对应产物卡）、运行日志（每次运行的模型、状态、耗时与按 sequence 排序的工具调用），带返回/关闭头部；作品概览头部去外框、统计改为 pill、移除与项目标签条重复的导航按钮。新增样式表统一执行 ≥12px 可读性规则。附 `WorkbenchSidePanel` 单元测试，更新工作台页与浏览器规格中的模板交互步骤。
- **自绘标题栏（无系统装饰）**：主窗口 `decorations: false`，顶部标签栏兼作标题栏——空白区 `data-tauri-drag-region` 支持拖动与双击最大化，右侧新增自绘最小化 / 最大化（还原） / 关闭按钮（`useWindowChrome`，按 `onResized` 同步最大化态；关闭经 `appWindow.close()` 进入既有离开保护），浏览器模式不渲染窗口控制。修复系统浅色模式下原生标题栏与应用深色主题不一致的问题。允许列表新增 `window.minimize / maximize / unmaximize / startDragging`，`Cargo.toml` 同步 `window-*` features（`cargo check --locked` 通过，锁文件不变）；`@tauri-apps/api/window` 改为仅在 Tauri 环境懒加载。
- **资源中心与详情卡片规格统一（P1 第二批）**：`page-layout.css` 新增资源页组件规格（`resource-grid / resource-card(--flow/--roomy/--interactive/.is-active) / resource-card-header / resource-card-title / resource-pill(--primary/--success/--warning/--error) / resource-meta / resource-actions / resource-footer / resource-toolbar / resource-form(-grid) / resource-editor / resource-pre / resource-notice / resource-empty / resource-list / resource-stat-card / resource-trace` 等），风格方案、模板中心、创作资产、导入导出、AI 任务记录五页及其对话框与卡片全部改为类驱动，移除约 130 处内联样式；AI 任务记录卡以 `data-status / data-selected` 驱动左侧状态条与选中态，`.btn-xs` 首次获得样式定义（12px / 28px 最小高度）。设置页新增 `settings-card-*` / `settings-subcard / settings-toggle-row / settings-checkbox / settings-result / settings-mode-banner / settings-danger-*` 规格，`AiRuntimeOverviewCard` 去掉硬编码的浅色十六进制底色与边框（深色主题下不再出现白色块），四个引擎状态格改为数据驱动的 `runtime-tile`。作品详情卡片（作品信息 / 世界背景 / 规则体系 / 主角设定及其字段与展示 / 角色库）改用 `novel-detail.css` 的 `detail-card-header / detail-card-title / detail-form(-grid) / detail-list-item / detail-subsection-title / detail-textarea / detail-save-status / detail-empty-hint / detail-avatar` 等类，去除全部内联样式（约 106 处）。页面标签激活色改为中性文字色以贴合 ZCode 形态。`ReadableSupportStyles` 测试新增 `page-layout.css / hub.css` 的 ≥12px 巡检与 28px 图标按钮尺寸断言；`ReadableInlineSupportText` 对模板卡与编辑表单改为断言类契约。所有 `data-testid`、可访问名称、路由与业务逻辑不变。
- **章节审阅右侧工具轨对齐新壳层（P1-8）**：右侧工具轨宽度与全局图标轨统一为 56px（`--right-toolbar-width`，2K/4K 断点同步 56/60px），按钮 44px、激活态改为中性选中底色 + 细指示条（不再使用强调色）；审阅面板宽度与工作台右侧面板统一为 360px（`--right-panel-width`），底色、44px 头部（14px 标题 + 28px 关闭）与 12px 内边距均按 `.workbench-side-view-header` 规格；面板内分区标题去掉大写与字距。Toast 右侧偏移随新轨宽调整；浏览器布局规格编辑区宽度断言按 56px 轨更新；`ReadableSupportStyles` 新增 `right-dock.css` 巡检与轨/面板宽度与壳层令牌一致性断言。
- **视觉收尾：浅色主题巡检与浏览器基线重标（P1-9）**：全量扫描样式表与生产 TSX 中的硬编码颜色——样式表侧仅剩窗口关闭键的 Windows 标准红色（有意保留）；生成追踪面板的 Fallback 告警框与记忆检查面板的告警/错误文字由浅色专用十六进制改为 `--color-warning-*` / `--color-error-text` 语义令牌，深色主题下不再出现浅色块；26 个面板/编辑器文件中 111 处 9～11px 内联字号统一提升到 12px 下限。故事资产页卡片改用 `--color-bg-card` + 12px 圆角、标签激活色改为中性文字色。浏览器 E2E：`story-assets` 规格的主题令牌断言按 ZCode 令牌重标（深色侧栏 `#1a1a1a` / 文字 `#ececec` / 卡片 `rgb(35,35,35)`，浅色 `#f7f7f8` / `#18181b` / `rgb(255,255,255)`），`writing-workspace-layout` 编辑区宽度按 56px 右侧轨更新；`theme-visual-baseline.json` 的亮度区间无需调整，8 个浏览器规格全部通过。
- **结构化应用策略收敛（审计 GAP-10 前端部分）**：新增 `src/services/conversation/structuredApplyPolicy.ts` 作为 `request_apply` 的单一策略源——白名单类型、只读报告、上下文压缩可应用条件、章节作用域与目标解析、`planStructuredApply` 目标复验——`artifactDecisionService.applyStructured`、`workbenchConversationDecisionService`、`workbenchHelpers.resolveArtifactDecisionTarget` 与产物卡片全部改为委托，去掉四份各自维护的规则副本；结构化产物绝不回退到当前打开的章节。Rust `validate_static_scope` 白名单与 `expected_target` 不变，仍是最终权威。附矩阵单测。
- **Writing SubAgent 契约设计稿与脚手架（审计 GAP-01/09，Gate E-0）**：新增 `docs/architecture/writing-subagent-contract.md`，定义 candidate-only 的 Writing SubAgent 契约（独立身份、只读工具 + 单一候选工具的 allowlist、独立模型快照、候选/字数/完整性修复预算、取消、错误责任边界、脱敏证据）与 E-0～E-4 分步接入方案；新增 `src/services/agents/writingSubAgentContract.ts`（`planWritingSubAgentTurn / assertWritingSubAgentContract / isWritingSubAgentDshEnabled`）及 5 组不变式单测。特性开关 `ai_novel_studio_writing_subagent_dsh` 默认关闭，`chapter_write` 仍走确定性 Writer，生产行为零变化；真实模型验收（E-4）本轮不执行。
- **Writing SubAgent E-1～E-3（仍默认关闭）**：Rust DSH 运行时新增任务类型 `chapter_write`（→ `generate_chapter`）与 `chapter_polish`（→ `polish_chapter`），必须绑定章节，allowlist 收窄为四个只读工具 + 唯一候选工具，宿主指令写入正文写作/润色要求与字数区间；`StartTaskTurnInput` 新增 `chapterWordRange`（只允许章节写作任务携带且需 `minimum ≤ target ≤ maximum`），候选落库前用与 `draft_service::word_count` 同源的统计做字数校验，越界返回 `DSH_CHAPTER_CANDIDATE_LENGTH_REJECTED`。TS 侧 `taskSessionAdapter`：仅桌面端 + 开关开启 + 已绑定章节时，`chapter_write` 意图经契约规划器投影为 DSH start 契约；回合返回 `chapter_text` 后运行与 Writer 同源的 `inspectChapterCandidateIntegrity`，有 `error` 级问题时追加助手回合列出问题码并建议「要求修改」，候选不改不删。Rust 3 项、TS 适配器 3 项 + 契约 1 项单测；开关关闭路径经单测证明零变化。
- **Writing SubAgent E-4a 真实模型正向验收（客户端 Gemini 卡片，PASS）**：新增 opt-in 真实配置验收载体 `tests/real-acceptance/wdio.real-profile.conf.ts` + `writing-subagent-real-profile.spec.ts`（`npm run test:real-profile:writing-subagent`，需 `AI_NOVEL_STUDIO_REAL_PROFILE=1`、先关闭桌面应用、非提权 shell）：用 tauri-driver 启动生产 EXE 并复用操作者真实配置——已保存的模型卡片与 DPAPI 凭据保管库由应用自行解密，密钥不出应用；不设隔离 E2E 标记（它会阻断网络并改写数据目录），WebView2 用户数据目录传 `%LOCALAPPDATA%\com.ainovelstudio.app`（传子目录 `EBWebView` 会得到嵌套的空配置）。规格经生产 IPC 建立固定作品「Writing SubAgent E-4 真实验收」（世界/规则/主角/卷/章纲齐备，后续运行复用并新增章节），在 WebView 内写入开关，经生产工作台 UI 创建并启动任务，结束后取证并清除开关；证据只含工具名、状态、计数、长度与哈希（`test-results/real-profile/<runId>/`），不含提示词、正文与凭据。2026-09-08 用 `openai_compatible:gemini-3.7-flash-high` 两次运行一致：1 个 run `completed`（25～30 s），`novel.read_context → chapter.read_outline → get_character_states → search_memory → generate_chapter` 全部成功且 allowlist 外模型工具调用为 0，`chapter_text` 候选 `valid` 且绑定目标章节，汉字数 1008 / 1122 落在硬区间 800～1150，章节字数/草稿数/作品总字数零变化，开关运行后恢复。`dsh.session.title` 为运行时会话事件投影（`source=dsh-session.event`），不是模型工具调用。E-4b（跨书负例、重启恢复、失败重试、越权负例）未做，开关仍默认关闭。
- **Writing SubAgent E-4b 负例与恢复验收（故障注入，PASS ×2）**：新增 opt-in 故障注入载体 `tests/real-acceptance/wdio.fault-injection.conf.ts` + `writing-subagent-fault-injection.spec.ts`（`npm run test:fault-injection:writing-subagent`，需 `AI_NOVEL_STUDIO_FAULT_INJECTION=1`、非提权 shell）：仍启动生产 EXE 并走真实 DSH 运行时、Gateway、Rust 宿主与工作台 UI，但把 `LOCALAPPDATA / APPDATA / WebView2` 用户数据目录重定向到运行目录下的隔离配置（不碰操作者的设置、保管库与数据库，DSH 载荷经 `DSH_RUNTIME_ROOT` 只读复用），模型由回环脚本上游 `scripts/dsh/mock-workbench-upstream.mjs` 扮演——新增故障模式 `forbidden-tool`（调用 allowlist 外的 `expand_settings`）、`cross-novel`（候选携带另一部作品 id）、`upstream-error-once` / `upstream-error`（HTTP 500 一次 / 持续）、`hold-generate`（挂起候选补全供强杀进程），进程内 `configure()` 可在同一端口切换模式与 id，且只按最新用户消息之后的工具调用推导下一步（对应宿主“必需读取须在本回合完成”）；两个生产载体共用 `production-carrier.ts` / `production-app-helpers.ts`。零网络、零凭据、零计费。六个场景全部通过并保持正式章节字数 / 草稿 / 作品总字数零变化：越权工具被宿主拒绝投影、run 失败零候选；跨书候选被 Gateway 作用域校验拒绝（`DSH_EXPECTED_CANDIDATE_FAILED`）且另一部作品不变；瞬时 500 由传输层自动重试并在同一 run 内产出唯一候选；持续 500 三次后 run 失败关闭，上游恢复后显式重试成功；88 字候选被 `DSH_CHAPTER_CANDIDATE_LENGTH_REJECTED` 拒收；候选补全挂起时强杀进程，重启后恢复对话框回收 1 个中断 run，重试在新 run 内重新完成全部读取后成功、无残留 worker。开关仍默认关闭；两条产品发现登记为 GAP-18（DSH 运行未持久化章节目标，目标文本不含章节标识时失败运行无法重试）与 GAP-19（重启后重试复用会话转录，模型若沿用旧读取会被 `DSH_REQUIRED_CONTEXT_READ_MISSING` 拒收），均已在本版修复（见「修复」）。

### 修复

- **任务运行持久化章节目标，失败运行可安全重试（审计 GAP-18，migration 038）**：此前桌面 `task_runs` 没有章节列，Rust `CreateRunInput` 丢弃前端传入的 `chapterId`，DSH 工具投影只保留 `chapterIdHash`；DSH 运行若在任何工具调用或候选产生前失败（上游持续失败、进程被杀），重试只能靠回合目标文本定位章节，目标写成“生成本章正文…”时被 `WORKBENCH_RETRY_TARGET_MISSING` fail-closed 拒绝。新增 `038_task_runs_chapter_binding`：`task_runs.chapter_id`（可空）、条件索引，并把章节纳入运行不可变身份触发器；`create_run` 接受 `chapterId` 并校验章节属于对话所在作品且未删除（`TASK_RUN_CHAPTER_SCOPE_MISMATCH`，拒绝即不创建运行）；DSH 运行创建时写入 `StartTaskTurnInput.chapter_id`，确定性 Writer 路径经 `create_task_run` 传入同一字段；会话包序列化 `chapterId`，前端 `workbenchRetryTarget` 的 `run.chapterId` 证据源直接生效。项目备份行校验新增可选 `chapter_id`，恢复随通用引用列重映射，旧备份为 NULL，备份 schema 不变。迁移账本 checksum 用例同步；`docs/data-model.md` 新增第 42 节。
- **中断/失败后的重试显式要求本回合重新读取（审计 GAP-19）**：DSH 会话按对话共享，重试会复用被中断运行的转录，而宿主的“必需读取”按 run 计数——模型若沿用转录中的旧读取直接提交候选，会以 `DSH_REQUIRED_CONTEXT_READ_MISSING` 失败关闭并需要用户再次重试（故障注入 `e4b-04` 复现）。宿主现在在创建 run 前统计同一回合已有的失败/取消运行数，用户重试时在回合提示追加“用户重试”说明（此前回合读取已失效、宿主只承认本回合内完成的读取、必须重新并行完成全部必需读取后再进入候选阶段）；自动协议恢复提示不变，按 run 校验的安全属性不变。回环 mock 以布尔值记录该说明是否到达模型，故障注入验收 S3b/S4 证实原运行请求均不含、重试运行每个请求均含；验收目标文本改回不点名章节，重试均一次成功且两个 run 绑定同一章节。
- **DSH supervisor 观察者错误保留根因**：`session.event` 投影失败时此前“最后一个错误覆盖前一个”，越权工具调用的 `DSH 调用了未授权工具: expand_settings` 会被随后到达的 `tool/result 找不到对应调用` 覆盖，运行错误只剩下游症状。现改为首个错误优先（`record_observer_error` + 单测），E-4b 故障注入验收中越权场景的 run 错误已直接指向未授权工具。

- **工作台右侧面板三个新视图排版错位**：`创作上下文 / 本任务产物 / 运行日志` 打开时面板被排到左侧任务树下方并压成 248px——三列栅格只对 `data-side-panel='launcher' | 'plugins'` 生效，新增视图落回两列栅格后换行。现改为任何非 `closed` 的面板状态都占第三列；窄于 1180px 的窗口保持两列并让面板以 360px 覆盖在对话流右侧，不再压扁对话流。侧面板内的创作上下文改为纵向单列分组卡（不再沿用输入区里的四列横排与 238px 高度上限），产物列表补齐 `quality_report / style_analysis / generic_text` 的中文类型标签，关闭按钮补 `workbench-side-close` testid。新增浏览器布局规格「三个视图在 1440 宽停靠右列、1024 宽覆盖」与 CSS 契约单测（同时防止再给面板加 `grid-row`——显式行号会让自动放置把面板排到第 1 列）。
- **大纲写命令作用域校验（审计 GAP-11）**：`save_master_outline / save_volume_outline / save_chapter_outline` 此前不校验 `project_id` 是否存在，也不校验 `volume_id / chapter_id / master_outline_id / volume_outline_id` 是否属于该作品，且非事务、版本号查询错误被 `unwrap_or(0)` 吞掉，可能把另一部作品的章节纲要挂到当前作品下。现改为 `Immediate` 事务内显式复验（作品存在且未删除；卷/章节属于同一作品；上级纲要属于同一 `project_id`），不匹配返回 `OUTLINE_SCOPE_MISMATCH: …` 且零写入；更新语句加 `project_id` 守卫；无激活版本时的非新版本保存改为 `MAX(version)+1`，不再复用版本 1。新增 5 个 Rust 负例/单调性测试。
- **TXT 导入改为单一事务（审计 GAP-16）**：此前导入对话框逐条调用 `create_novel / create_volume / create_chapter / save_chapter_draft_atomic`，任一步失败会留下半导入作品。新增 Rust `import_txt_novel` 命令（`services/txt_import_service.rs`）：作品、卷、章节与 `source=imported` 的未采用草稿在一个 `Immediate` 事务内落库，超过 100KB 的章节走既有分块大文本存储；入参先做标题/章节/序号/长度校验（`TXT_IMPORT_INVALID`），任何失败零部分写入。前端 `txtImportService.importTxtNovel` 桌面端直接调用该命令，浏览器开发模式回退为逐步写入并在失败时级联删除已创建作品作为补偿。新增 Rust 3 项（含存储失败回滚）与 TS 3 项单测、桌面 E2E `txt-import-atomic.spec.ts`（第 22 个场景，经生产对话框投递两章 TXT 并核对章节与草稿事实），变更选择器登记 `TXT import transaction` 归属。
- **导入资产与润色记录改以 SQLite 为桌面事实源（审计 GAP-15 第一批）**：`imported_assets / polish_records` 两张表此前无任何命令写入，前端一直把 LocalStorage 当事实源，备份里这两张表在桌面端永远为空。新增 `commands/local_assets.rs`：`list/save/delete_imported_asset` 与 `list/get/create/update/delete_polish_record`，保存时复验作品、章节、来源/结果草稿、AI 任务归属与枚举值（`LOCAL_ASSET_INVALID`），按 id 幂等 upsert。`importedAssetService / polishService` 桌面端改走命令，浏览器开发模式保留 LocalStorage；首次读取时把历史 LocalStorage 记录幂等迁入 SQLite（引用已不存在对象的孤儿记录跳过，不阻断），迁移标记 `ai_novel_studio_imported_assets_sqlite_v1 / ai_novel_studio_polish_records_sqlite_v1`。Rust 2 项、TS 6 项单测。
- **用户模板与设定建议改以 SQLite 为桌面事实源（审计 GAP-15 第二批，migration 037）**：新增 `037_user_templates_and_setting_suggestions` 建表 `user_templates`、`setting_suggestions`（枚举与 JSON 形状 CHECK、作品外键、状态一次性推进与身份冻结触发器），迁移账本 checksum 用例同步。新增 `commands/user_templates.rs`（list / save / delete，按 id 幂等 upsert、更新保留 `created_at`、标签变量去重限长）与 `commands/setting_suggestions.rs`（list / get / 批量 save 同事务 / decide CAS）。前端 `templateService` API 改为异步并同步模板中心页调用；`settingSuggestionService` 的候选读写经统一 `suggestionStore` 走命令，采纳/放弃只能从 `pending` 推进一次；两者浏览器模式保留 LocalStorage，首次桌面读取幂等迁入历史记录。`docs/data-model.md` 新增第 41 节。Rust 4 项 + 迁移套件 19 项、TS 3 项单测。
- **UI 全面体验收口**：完成作品/资产加载与错误分层、资产统计作用域隔离、任务筛选反馈、长目标输入与模板撤销、中文输入法保护、统一模态焦点、章节审阅专注模式、正文信息收敛、通知暂停与队列、任务/章节快速定位及大纲候选主动定位；保留原有 SQLite、Provider、授权、CAS 与事务边界。
- **共享弹窗与表单样式**：补齐新建作品、卷章、风格/输出方案、设定编辑及恢复/离开提示共用的弹窗容器和字段样式，恢复居中、限宽、主题底色与长表单滚动；保留导入弹窗的独立定位及宽版设定编辑。共享卡片、字段标签和选择框样式改为启动时加载，作品详情紧凑按钮仅作用于本页，避免访问顺序改变其他页面外观；不改创建、保存、导入或确认业务。
- **候选审阅与应用语义**：结构化候选保留只读原文，“已审阅”明确为本地标记，编辑字段改为独立修订意见；应用前常驻说明整份原始候选范围。两种修订入口均带出本卡全部已填意见，追加而不覆盖原任务草稿，保护等待期间输入、任务切换与决定成功后的刷新失败；不扩展局部应用，不改变后端事务、产物哈希或授权边界。
- **项目备份边界**：完整备份的本地附件由共享白名单和项目归属规则在 TypeScript/Rust 写入前复验；限制恢复键和 ID 重映射，保留失败补偿，补齐 schema 10 缺少后续决定表的兼容分支。未知或越界附件整体拒绝，不改备份 schema 和既有 migration。
- **任务目录可达性**：桌面/浏览器统一按稳定时间与 ID 游标分页，搜索和归档筛选先于数量限制；支持加载更多、搜索竞争隔离及按 ID 恢复首屏以外的最近任务。
- **指令与长篇约束**：否定动作、操作对象和引用正文分离；含糊目标在模型调用前要求澄清。完整创作要求与有界 Memory query 分离，持续约束最新优先并展示纳入、替代和预算遗漏的脱敏回执。
- **质量误判与凭据告知**：低置信语义提醒随候选进入审阅，不自动重写，硬完整性门禁保持；设置与错误提示区分 Windows DPAPI 加密保存、重启恢复和浏览器仅会话模式。
- **测试与发布门禁**：补入遗漏单测并检查 runner 归属；静态 SSR 测试关闭无关文件监听和依赖扫描，避免大型桌面构建目录导致验证挂起。当前 Canonical 文档及桌面探针与 Manifest/宿主一致性可自动校验。更新通道由共享 SemVer 派生并串行发布，防止版本倒退，校验不可变产物与严格前版本回滚，部分上传失败执行补偿并复验；不将模拟验证宣称为远端发布成功。
- **测试矩阵去重与归属门禁**：`npm test` 不再重复执行与 `test:workbench` 重叠的 6 个工作台服务测试（会话失败/模型可用性/章节写作、采纳稿记忆、工具注册表、Main Agent 运行时），现仅在 `test:workbench` 运行一次；`test:ownership` 对默认链路内同一测试的重复归属失败关闭并附自测试。`verify_project.ps1` 的 `cargo check` 补齐 `--locked`。不改变测试断言、覆盖率阈值或发布门禁集合。
- **开发验证分层、重复执行与真实场景覆盖**：新增 `npm run verify:change`（`scripts/quality/verify-change.mjs`，支持 `--base <ref>`、`--dry-run`、`--lane`、`--full`、`--report`），按 `scripts/quality/verification-scopes.mjs` 的模块归属表把已暂存/未暂存/未跟踪变更路径映射到行为测试、Rust 筛选与桌面场景，合并去重后输出选择原因、命令、用例数、耗时与结果；未映射路径、零行为测试、零执行用例与任一失败均失败关闭，不自动扩大为全仓检查，完整门禁吸收定向选择避免重复。`run_feature_workflow.ps1` 默认只做只读开工检查，`-Phase Verify` 调用选择器并原样传播退出码，不再无条件调用完整发布矩阵。执行层：`run_workspace_test_suite.ps1` 三个 Rust 专项改为经 `scripts/quality/run-cargo-tests.mjs --exact` 按完整测试名精确执行（保留非零匹配、`--locked` 与 `--test-threads=1`），不再丢掉筛选运行全部 Rust 测试；`verify_project.ps1` 以 `--list-only` 校验 AI Task 删除与项目备份用例存在，实际执行由完整 `cargo test` 覆盖；`test:coverage` 保持“C8 采集 `test:all` → 核心集合报告 → `test:coverage:components` 关键组件门禁”的链路（曾尝试让 `test:vitest` 直接启用 Vitest 覆盖率提供者以免重跑组件测试，但它会接管子进程的 `NODE_V8_COVERAGE`，在 Linux CI 上使 C8 丢失 Vitest 用例贡献、核心集合从 87% 跌到 78%，已回退），阈值与归属检查保持不变。桌面验收：`isE2eLegacyWorkspacePanelsEnabled()` 要求 E2E 构建加显式 localStorage 开关，运行器只为 `scripts/e2e/spec-selection.ts` 列出的兼容性用例注入 `AI_NOVEL_STUDIO_E2E_LEGACY_PANELS=1`，默认 E2E 保持生产界面；新增 `workbench-writing-smoke.spec.ts` 在真实 Tauri、隔离 SQLite 与固定模型响应下完成创建作品和章节 → 工作台任务 → 生成候选 → 请求修订 → 显式审阅 → 编辑保存 → 确认采用 → 真正重启进程 → 核对正文、采用记录与授权状态（Mock 总结失败记为失败边界），`test:e2e:smoke` 改为 `app-start` + 该场景，`--spec` 可重复选择多个场景并统一构建一次，逐场景数据库隔离与进程清理不变；运行器启动前检测 Windows 提权终端并立即失败（WebView2 Runtime 150+ 在 High integrity 宿主中丢弃 `WEBVIEW2_*` 覆盖，msedgedriver 无法注入远程调试端口，否则每个 spec 静默超时约 2 分钟），`AI_NOVEL_STUDIO_E2E_ALLOW_ELEVATED=1` 为显式旁路；`RightToolbar.test.tsx` 同步为“E2E 构建标记本身保持生产工具栏、仅显式兼容开关恢复旧面板”的契约。CI：PR 在 Linux `ci.yml` 以 `--lane frontend`、在 Windows 以 `--lane native` / `--lane desktop` 使用同一选择规则，不适用时输出 `NOT_APPLICABLE`；`main` 推送、定时、手动完整模式与 `workflow_call` 保留完整门禁；前端车道在 Linux 执行完整 `test:all` 矩阵，C8 覆盖率阈值（`test:coverage`）只在校准过的 Windows 工具链（选择器按平台切换，Windows 发布门禁 `verify_project.ps1`）执行——Linux 上 C8 对 `tsx --test` 子进程的覆盖归因偏低，同一批测试的核心集合会低约 10 个百分点；`release.yml` 复用同一提交成功的完整前置门禁并复验 `verified_sha`，签名、产物与通道检查独立执行；required check 名称不变。同步 `AGENTS.md`、测试/桌面 E2E/开发规则/Git 治理文档、`.github` instructions/prompts/checklists/skills、PR 模板与 Cursor 规则：区分日常修改、专项验收和发布，移除重复确认、每改一个文件即检查与失败后无条件重跑全部门禁的要求，专项 Cursor 规则改为按文件范围或任务启用，修正过时的 Node 要求、窗口尺寸与构建命令。选择器、Rust runner、spec 选择与旧面板门禁均附行为测试。

### 文档

- **Agent 主入口优化**：精简 `AGENTS.md`，集中开工检查、按任务查阅、文档权威、模块落点、安全底线、分层验证与完成汇报；明确保留用户已有修改、相应 Git/发布动作需授权、纯文档不自动升级版本。
- **开发指令一致性**：同步 Agent 工作流、开发规则、Copilot/行为指令、Cursor 规则及 docs-sync Skill；消除无条件确认/提交/构建、错误工作目录、过时主题/数据库路径和预留 schema 暗示，将任务规则占位页改为可执行速查。
- **文档导航与事实校准**：README 与文档索引补齐 Agent 入口和时效导航；按现有 Manifest/宿主与凭据代码校正四项 Canonical 只读准入、R4 live 未验收及 Windows DPAPI 持久化边界，并修正桌面构建入口。本次仅更新文档，不实现新能力、不改变应用版本或发布状态。

### 验证

- `npm run lint:ci`、`tsc --noEmit`、`npm run test:component-size`、`npm run test:bundle-size`
- `npm run test:coverage`（内含完整 `npm run test:all`：ownership、node 套件、ai-panels、vitest 62 文件、performance、workbench、discovered；全局行覆盖 60.4%，核心 87.8%，关键组件门禁通过）
- `cargo test --locked --manifest-path src-tauri/Cargo.toml -- --test-threads=1`（496 passed / 2 ignored；含 migration 037/038 账本与章节绑定、重试提示、supervisor 根因保留用例）
- `npm run test:docs-sync`、`npm run test:version-sync`、`node scripts/quality/check-capability-docs.mjs`、`npm run test:ownership`、`node scripts/quality/verify-change.mjs --dry-run`、`npx prettier --check`（本版全部改动文件）、`git diff --check`
- `npm run test:e2e`（真实 Windows Tauri，全部 22 个独立桌面场景通过）；`npm run test:e2e:browser`（8 个浏览器规格通过）
- opt-in 真实验收：`npm run test:real-profile:writing-subagent`（客户端 `openai_compatible:gemini-3.7-flash-high`，E-4a PASS；开放后按默认路径复验）；`npm run test:fault-injection:writing-subagent`（E-4b 六场景，修复 GAP-18/19 后连续两轮 PASS）
- 生产 EXE `src-tauri/target/release/AI Novel Studio.exe` 重建（v3.7.0）
...
- **单人维护仓库发布治理死锁**：常规发布继续要求非作者审查；当仓库仅有一名具备写权限的维护者、GitHub 无法提供自我 `APPROVED` 时，允许以当前 PR head 全部门禁成功、无反对审查、对话清零、完整发布证据和显式例外记录替代非作者批准。PR 合并、`main` 同步和不可移动 tag 的顺序保持不变。
- **模型卡当前选择与取消隔离**：云端、本地和 Gateway 删除非当前模型卡时继续保持原 active 模型；首次尚无云端模型卡时，编辑器取消只恢复进入编辑前的 Cloud 字段，并保留编辑期间切换的运行模式及其他模型设置，后续通用保存不会把已取消的 Endpoint、模型名或会话 Key 重新形成模型卡。
- **工作台候选与启动晚到状态协调**：审阅候选使用同步引用阻止 3 秒章节树刷新把候选替换为普通草稿；启动恢复任务超时后若底层操作最终成功，会把对应快照重新协调为 `succeeded`，后续 readiness 等待可直接重试而不永久停留在失败态。
- **项目删除与章节章纲事实一致性**：兼容 `delete_novel` 恢复为可恢复软删除并保留独立级联物理删除入口；结构化章节章纲在同一事务内同步 `chapter_outlines` 与 `chapters.outline/updated_at`，零行命中时整体回滚。
- **历史人物状态按正式卷章顺序投影**：人物状态历史与 `current_state` 改按卷序、章序和稳定兜底键选择权威最新事实；晚回填旧章总结不再覆盖后续章节状态，后续章节提示上下文仍读取最近前态。
- **草稿历史提示计时器生命周期收口**：草稿采用与废弃后的瞬态提示统一复用单一计时器，并在面板卸载时主动清理；避免 Windows 覆盖率或用户快速关闭面板后，延迟回调继续更新已销毁的 React 视图。
- **浏览器正文保存瞬态探针稳定化**：正文内联保存 E2E 直接观察右侧保存按钮自身的 `aria-busy` 与标签字符变更，并通过 MutationObserver 旧值保留快速 `saving → saved` 提交中的“保存中”证据；不再从状态栏回调间接采样已经回到终态的工具栏，也不为测试延长真实保存耗时。
- **Windows DSH start-path 文件库隔离**：Rust 单测中的真实 DSH Worker/Gateway 启动链复用既有的测试专用 SQLite 路径覆盖，四个 start-path 场景在各自临时目录创建完整文件库并通过 RAII 恢复 Runtime、Worker、Gateway 与数据库环境；干净 Windows Runner 不再因测试权威内存库与外部 Gateway 默认用户库错配而在首次模型请求前断开，生产构建仍固定读取真实应用数据库。
- **桌面闭环 E2E 不可变修复产物计数**：五轮生产闭环明确区分对话中可审阅的最终候选卡与完整性修复前保留的不可变审计产物；精确验证 10 个 Run/Card，并允许跨章修订门禁保留至多 1 个未挂卡修复前产物，避免把正常审计轨迹误报为重复生成，同时继续拒绝额外产物漂移。
