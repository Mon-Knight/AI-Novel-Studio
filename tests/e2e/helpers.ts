...
export async function bridgeCall<T>(command: string, args?: Record<string, unknown>): Promise<T> {
  const response = await browser.executeAsync((name, input, done) => {
    const bridge = (window as unknown as { __AI_NOVEL_STUDIO_E2E__?: BridgeShape }).__AI_NOVEL_STUDIO_E2E__;
    if (!bridge?.invoke) return done({ ok: false, error: 'window.__AI_NOVEL_STUDIO_E2E__.invoke is unavailable' });
    let settled = false;
    ...
    const timer = window.setTimeout(() => finish({ ok: false, error: `timeout invoking ${name}` }), 15000);
    Promise.resolve(bridge.invoke(name, input))
      .then((value) => { window.clearTimeout(timer); finish({ ok: true, value }); })
      .catch((error) => { window.clearTimeout(timer); finish({ ok: false, error: String(error) }); });
  }, command, args ?? {});
  const result = response as { ok: boolean; value?: T; error?: string };
  if (!result.ok) throw new Error(`E2E bridge invoke ${command} failed: ${result.error ?? 'unknown error'}`);
...
export async function bridgeDiagnostics(): Promise<Diagnostics> {
  const result = await browser.executeAsync((done) => {
    const bridge = (window as unknown as { __AI_NOVEL_STUDIO_E2E__?: BridgeShape }).__AI_NOVEL_STUDIO_E2E__;
    if (!bridge?.getDiagnostics) return done({ ok: false, error: 'E2E diagnostics bridge is unavailable' });
    let settled = false;
    const finish = (value: { ok: boolean; value?: unknown; error?: string }) => {
      if (settled) return;
      settled = true;
    ...
    const timer = window.setTimeout(() => finish({ ok: false, error: 'timeout invoking get_e2e_diagnostics' }), 15000);
...
export async function assertCleanDiagnostics(): Promise<void> {
  ...
  const frontEndResult = await browser.executeAsync((timeoutMs, done) => {
    ...
    Promise.all([
      ...
      .then(([errors, logs]) => finish({ ok: true, value: { errors: errors ?? [], logs: logs ?? [] } }))
...
export async function findTestIdByAttribute(
  ...
  const find = async () => {
    const elements = await browser.$$(`[data-testid="${testId}"]`);
    for (const element of elements) {
      if (await element.getAttribute(attribute) === expected && await element.isDisplayed()) return element;
    }
    return undefined;
  };
...
export async function navigateHash(route: string): Promise<void> {
  const expectedHash = route.startsWith('#') ? route : `#${route.startsWith('/') ? route : `/${route}`}`;
...
export async function createChapterThroughUi(title = unique('Chapter'), volumeId?: string): Promise<string> {
