...
describe('chapter editing', () => {
  it('creates a chapter, edits its draft, and persists the saved content', async () => {
    ...
    expect(await volume.getAttribute('data-volume-title')).toBe(
      E2E_FIXTURES.chapterSave.volumeTitle,
    );
    ...
    await browser.waitUntil(
      async () => {
        ...
        return (
          (await currentEditor.getAttribute('data-dirty')) === 'false' &&
    ...
    const chapters = await bridgeCall<Array<{ id: string; novelId: string; volumeId?: string }>>(
    ...
    const drafts = await bridgeCall<
      Array<{ id: string; novelId: string; chapterId: string; content: string }>
    >('get_drafts_by_chapter_id', { chapterId });
    ...
    const persistedDrafts = await bridgeCall<Array<{ chapterId: string; content: string }>>(
      'get_drafts_by_chapter_id',
      { chapterId },
    );
    expect(
      persistedDrafts.filter((draft) => draft.chapterId === chapterId && draft.content === content),
    ).toHaveLength(1);
  });
});
