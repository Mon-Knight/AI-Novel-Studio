...
describe('unsaved document leave guard', () => {
  it('keeps dirty content when leaving is cancelled, then allows the confirmed switch', async () => {
    ...
    const secondChapterId = await createChapterThroughUi(
      E2E_FIXTURES.leaveGuard.secondChapterTitle,
    );

    const firstChapter = await findTestIdByAttribute(
      'chapter-item',
      'data-chapter-id',
      firstChapterId,
    );
    ...
    const secondChapter = await findTestIdByAttribute(
      'chapter-item',
      'data-chapter-id',
      secondChapterId,
    );
    ...
    expect(
      await (
        await findTestIdByAttribute('chapter-item', 'data-chapter-id', firstChapterId)
    ...
    expect(
      await (
        await findTestIdByAttribute('chapter-item', 'data-chapter-id', secondChapterId)
      ).getAttribute('data-active'),
    ).toBe('true');

    const chapters = await bridgeCall<Array<{ id: string; novelId: string }>>(
      'get_chapters_by_novel_id',
      { novelId: projectId },
    );
    expect(
      chapters.filter((chapter) => chapter.id === firstChapterId || chapter.id === secondChapterId),
    ).toHaveLength(2);
    expect(chapters.every((chapter) => chapter.novelId === projectId)).toBe(true);
    const firstChapterDrafts = await bridgeCall<
      Array<{ id: string; chapterId: string; content: string }>
    >('get_drafts_by_chapter_id', { chapterId: firstChapterId });
    const savedDraft = firstChapterDrafts.find(
      (draft) => draft.chapterId === firstChapterId && draft.content === content,
    );
    expect(savedDraft).toBeTruthy();
    expect(
      firstChapterDrafts.filter(
        (draft) => draft.chapterId === firstChapterId && draft.content === content,
      ),
    ).toHaveLength(1);

    await (await findTestIdByAttribute('chapter-item', 'data-chapter-id', firstChapterId)).click();
    const reopenedEditor = await waitForTestIdAttribute(
      'chapter-editor',
      'data-chapter-id',
      firstChapterId,
    );
    ...
    const draftsAfterDiscard = await bridgeCall<
      Array<{ id: string; chapterId: string; content: string }>
    >('get_drafts_by_chapter_id', { chapterId: firstChapterId });
    ...
    await (await findTestIdByAttribute('chapter-item', 'data-chapter-id', firstChapterId)).click();
    const editorAfterDiscard = await waitForTestIdAttribute(
      'chapter-editor',
      'data-chapter-id',
      firstChapterId,
    );
