...
当前分支 `codex/v3.6.0-release` 已完成本地提交，仍处于发布候选：尚未创建 PR、合并到 `main`、创建 `v3.6.0` tag 或发布 GitHub Release。机器可读版本号表示候选目标，不等于版本已经发布。
...
- ✅ 对话式创作工作台、产物确认/审阅授权与写作工作台审阅收敛（v3.5.0）
...
- **协作事实与备份**：session、round、opinion 持久化到 migration 021～023，自主计划持久化到 migration 024；参考资料、混合语义 Memory、跨进程调度和正式故事资产分别由 migration 025～029 提供；任务对话、产物决定和审阅授权由 migration 032～036 提供。完整项目备份当前为 schema 11（schema 7 Memory、schema 8 Scheduler、schema 9 故事资产、schema 10 工作台对话事实、schema 11 产物决定/审阅授权均可恢复），并重映射全部身份。
...
- **三层 Novel Memory 实验算法**：长期/中期/短期片段、角色/世界状态和 `MemoryVersionSnapshot` 当前由 TypeScript 进程内 `Map` 管理，用于召回、状态演进和 A/B 评测实验；它不等同于 SQLite `memory_documents` 持久化事实，不能作为跨重启的正式小说记忆来源。
...
| v3.6.0                    | **本地发布候选，尚未发布**：生产边界纠偏、Canonical 能力资产化、工作台继续收敛与稳定性修复；Canonical 模型可见数量仍为 0，Main Agent 尚未放行                                |
