...
**阶段：章节工程真实 AI 请求取消闭环 - 传输中断、终态保护与桌面回归**

v2.1.6 让章节工程 `generation_jobs` 的正文生成和质量检查请求支持真实取消。桌面 API 模式改用异步 `reqwest`；用户取消时，前端 `AbortSignal` 会调用 Rust `cancel_ai_request`，确认中止仍在运行的 HTTP future 后，再以稳定错误码 `AI_REQUEST_CANCELLED` 结算。浏览器开发模式和 Mock Provider 使用同一取消语义，用户取消与超时保持可区分。

取消仍通过现有 SQLite 事务原子写入任务终态和唯一 checkpoint；迟到响应不能追加成功 step 或把任务改回完成。已经提交的草稿、报告等事实不会回滚。活动请求注册表有容量、TTL、重复 ID 和提前取消保护，异常退出时也会中止并清理请求。本版本不修改 migration，不把取消能力扩展到旧 AI 面板或所有历史 `ai_task_records`。
...
| v2.1.6 | **当前：章节工程真实 AI 请求取消闭环** |
| v2.x | 后续：质量历史、稳定重放与 Agent 能力增强 |
