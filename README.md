...
**阶段：章节工程任务跨重启恢复闭环 - 安全结算、结果保留与幂等恢复**

v2.1.5 会在应用启动时检查 `generation_jobs`：上次退出前仍处于 `pending`、`running` 或 `retrying` 的章节工程任务，会在一个 SQLite 事务中安全结算为 `failed`，记录稳定错误码 `APP_RESTART_INTERRUPTED` 并追加恢复 checkpoint。已完成步骤、当前进度、草稿、质量报告和 patch 结果均保留，应用不会自动重发 AI 请求或重放不确定步骤。

恢复操作可重复执行：第二次启动不会再次修改终态任务或追加重复 checkpoint。任务状态机同时拒绝终态复活和进度倒退，工作台会根据持久化任务状态禁用重复启动。v2.1.5 不修改 migration、不自动续跑、不扩展旧 `ai_task_records` 或真实网络取消。
...
| v2.1.5 | **当前：章节工程任务跨重启恢复闭环** |
| v2.x | 后续：可靠网络取消、质量历史与 Agent 能力增强 |
...
- AI 请求仍以完整响应为主；流式输出和可靠网络取消尚未完成。`generation_jobs` 已支持重启后的安全终结，但不会自动续跑不确定步骤。
