...
**阶段：章节质量历史不可变快照与原子重放闭环**

v2.1.7 把每次质量检查保存为独立、不可变的报告和问题快照。重复检出同一 `issue_key` 也不再搬移或改写旧记录；用户可以在质检面板回放历史报告，历史问题保持只读，当前报告的处理状态由独立工作流表管理。

报告、全部问题、当前问题状态与 `completed` 终态在同一 SQLite 事务内提交；任意一条失败都整笔回滚。新报告必须绑定产生结果的、归属匹配且已成功的 `quality_check` AI Task；迟到的旧报告可以完整保存自身快照，但不能重置较新完整报告的处理状态。完整备份升级为 `schemaVersion: 3`，并继续安全导入 schema 2。

已完成质量报告引用的 AI Task 现在作为追溯证据受保护，单删、混合批量删除或清空任务都不会再静默清除绑定。浏览器 LocalStorage 回退同样把历史 item 与当前状态分开保存；桌面回归会实际修改问题状态，并在应用重启后证明历史报告仍保持原始快照。
...
| v2.1.7 | **当前：章节质量历史不可变快照与原子重放** |
| v2.x | 后续：可证明安全的续跑、其他 AI 工具取消与 Agent 能力增强 |
