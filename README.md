...
v2.1.8 将桌面端章节上下文链路收敛到 SQLite 单一事实源。章节总结、上下文记录、角色状态、角色当前状态与章节 `summarized` 状态通过一个事务提交；任一归属校验或写入失败都会整笔回滚，并把错误返回给界面，不再静默写入 LocalStorage 或显示虚假成功。

上下文记录 ID 由调用方生成并在 SQLite 往返中保持稳定，桌面端支持完整读取、更新和过期；采用另一版正文会在同一采用事务中使旧总结与关联上下文过期，无需依赖打开面板触发。章节总结可按作品稳定查询每章最新记录。旧版 LocalStorage 中的总结、上下文和角色状态通过幂等迁移写入 SQLite，只有确认落库并完成映射的记录才会清理；歧义数据保留在本地并返回警告，重复迁移不会制造副本。

浏览器开发模式仍使用 LocalStorage，并在多步保存失败时执行补偿回滚；它不作为桌面发布的数据权威。版本号现在由自动同步门禁核对，统一验证入口同时运行 Node、Rust、真实 Tauri E2E 与生产构建；脏工作树不能获得发布建议。
