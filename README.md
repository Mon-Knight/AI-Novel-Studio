...
首次启动会优先呈现作品首页并显示本地作品读取进度；写作工作台、任务中心等较重页面按需加载。作品查询失败会显示可重试错误，不会误报为“没有作品”。
...
- **作品管理**：创建、编辑、删除小说作品，维护封面与基础元数据；总字数只聚合当前已采用正文，保存与重启后均从持久化数据重新计算。
...
- **安全导入导出**：通过系统文件窗口选择 TXT / JSON；桌面端使用受控 Rust 读取器校验扩展名、普通文件、64 MB 上限与 UTF-8 编码，不受用户目录范围限制，解析预览确认后再导入。章节 / 整本作品可导出为 TXT、Markdown，并支持项目 JSON 备份。
