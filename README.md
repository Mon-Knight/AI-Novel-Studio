...
- **协作事实与备份**：session、round、opinion 持久化到 migration 021～023，自主计划持久化到 migration 024；完整项目备份 schema 7 包含协作历史、自主计划、参考资料和混合语义 Memory，并重映射全部身份。
...
- 质量修稿会按全文位置只处理命中问题的分段；章节总结会覆盖完整正文并分层归并，章节改写与卷总结也会接收完整来源，不再只读取正文或章节列表前缀。
