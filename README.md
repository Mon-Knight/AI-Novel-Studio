...
**阶段：正文变更安全门 — 固定目标、版本校验、原子采用、会话级幂等应用**

v2.1.1 聚焦 AI 结果进入编辑器、草稿与正式正文时的安全边界：每次正文变更都绑定目标作品 / 章节和基础正文版本，迟到响应、版本冲突与重复应用不得静默覆盖当前正文。

此前 v1.9.5～v2.1.0 已完成章节工程状态、生成上下文快照、持久生成任务、正文初稿、结构化质检、局部修复和单章质量闭环。v2.1.1 不扩展新的 AI 自动写入能力，先收紧现有正文变更链路。
...
- v2.1.1 已引入动态前端与 Rust 安全测试，但 React 组件级并发覆盖和 Windows 桌面 E2E 仍不完整。
